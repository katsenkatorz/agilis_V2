...
//************************************************************************************
url = "//" + location.host + "/agilis_V2/public";
	
couleurP0 = "#FF6600";	//orange
couleurP1 = "#FFE0B2";	//orange clair
couleurS0 = "#000000";	//noir
couleurS1 = "#A9A9A9";	//gris foncé
couleurS2 = "#D3D3D3"; //gris clair
couleurS3 = "#FFFFFF";	//blanc

//************************************************************************************
// Fonctions globales à Agilis
//************************************************************************************

//Appel d'un controleur PHP avec retour d'un fichier XML à la fonction appelante
function ajax(controleur,action,variables) {
	var xhr = new ActiveXObject("Microsoft.XMLHTTP");
	xhr.open("POST",url + "/" + controleur + "/" + action,false);
	xhr.setRequestHeader("Content-type","application/x-www-form-urlencoded");
/*	
w = window.open();
w.document.write(url + "/" + controleur + "/" + action + '?variables=' +ax(arrayToXml(variables)));
w.document.write("</br></br>");
w.document.write(arrayToXml(variables));
*/
	xhr.send('variables=' + ax(arrayToXml(variables)));
	while(xhr.readyState != 4 || ! xhr.responseXML) setTimeout('',100); 
	return(xhr.responseXML);
}//end function

//Appel d'un contrôleur par Post
function post(controleur,action,variables) {
	xhr = new ActiveXObject("Microsoft.XMLHTTP");
	xhr.open("POST",url + "/" + controleur + "/" + action,false);
	xhr.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	xhr.send('variables=' + ax(variables));
}//end function

//Attente de la fin du chargement de tous les cadres de la page puis exécution d'une fonction
function apresChargementExecuter(fonctionExecuter){
...
function getVariableSession(variableNom){
	alert('Modifier this.getVariableSession() en formulaire.session[]');
//	return this.session[variableNom];
}//end function


//Affiche les variables de session
function sessionAfficher(){
	var message = '';
...
function devVariables(variables){
...
function nodeToArray(node,arrayNode){
	...
	while(node != null){
		if(node.nodeType == 4){
			...
			arrayNode[nodeName] = nodeToArray(node,arrayNode[nodeName]);				
		}//end if
...
function arrayToXml(listeArray){
	var listeXml = '';
...
function evaluerCrochets(chaine){
	var newChaine = "";
...
function evenementAjouter(elementId,evenementNom,evenementAction){
	...
	var setEvenement = "document.getElementById('" + elementId + "')." + evenementNom + "(){" + evenement + ";}";
	eval(setEvenement);
//alert(eval("document.getElementById('" + elementId + "')." + evenementNom))	;	
}//end function	

//Supprime une entrée d'un tableau
function unset(inputArray, index){
...
function formatVerifier(controleElement,format,option,blanc){
	var controleValeur = controleElement.value;
	var valeurSansFormat = '';
	if (controleValeur != "") {
		var erreur = false;
		if(format != 'date') valeurSansFormat = formatSupprimer(controleValeur,format,option);
		switch(format){	
			...
			case "date": 
				...
				valeurSansFormat = formatSupprimer(controleValeur,format,optionSaisie);
				if(! isDate(valeurSansFormat)) erreur = "une date";
				break;
		}//end switch
		if (erreur !== false) {
			...
			if(format != 'regexp'){
				controleElement.value = formatAppliquer(valeurSansFormat,format,option,blanc);
			}//end if
		}//end if
	}//end if
}//end function
...
function formatAppliquer(valeur,format,option,blanc){
	var resultat = '';
...
function formatSupprimer(valeur,format,option){
	var resultat = '';
	if (valeur == "") {
		resultat = "";
	}else{
...
function jourOuvrable(valeur,delta){
	...
	switch(nJour){
		case 0 : //dimanche
			jourOuvrable(dateOutput,2 * sens);
			break;
		case 6 : //samedi
			jourOuvrable(dateOutput,1 * sens);
			break;
	 }//end switch
	if (jourFerie.indexOf(dateOutput) > '-1') jourOuvrable(dateOutput,1 * sens);
