...
class Agilis_Model_Liste{
	...
	public function setListe($variables){
		...
		$this->liste['session']	= self::listeSession();	
		$this->liste['cadre'] = (isset($variables['var_cadre_code']) ? $variables['var_cadre_code'] : '');
		...
		$this->liste['valider'] = self::listeValider();
		$this->liste['supprimer'] = self::listeSupprimer();		
	...
	public function getListeXml(){
		$listeXml['session'] = $this->liste['session'];		
		$listeXml['regles'] = $this->liste['regles'];
		$listeXml['colonnes'] = $this->liste['colonnes']['alias'];
		return $listeXml;
	}//end function	

	private function listeSession(){
	...
	private function listeVariables($variables){
		...
		if($this->liste['proprietes']["zz_li_requetevariables"]){
			...
			//Remplacement éventuel des variables de session par leur valeur
			foreach($variablesDefaut as $variable => $valeur){
				if(substr($valeur,0,2) == '[['){
					$variableSession = substr($valeur,2,-2);
					$variablesDefaut[$variable] = $this->liste['session'][$variableSession];
				}//end if
			}//end foreach
		}//end if
	...
	private function listeLiens(){
		...
		foreach($this->liste['regles'] as $regle){
			$listeLiens[] = $regle['zz_rg_declencheurs'];
		}//end foreach
		return $listeLiens;
	}//end function	
	
	private function listeSupprimer(){
		$listeSupprimer = false;
		foreach($this->liste['regles'] as $regle){
			if($regle['zz_rg_declencheurs'] == 'icone_supprimer.onclick') {
				$listeSupprimer = true;
				break;
			}//end if
		}//end foreach
		return $listeSupprimer;
	}//end function
	
	private function listeValider(){
		$listeValider = false;
		foreach($this->liste['regles'] as $regle){
			if($regle['zz_rg_declencheurs'] == 'icone_valider.onclick') {
				$listeValider = true;
				break;
			}//end if
		}//end foreach
...
//			if($this->liste['colonnes']['proprietes'][$iColonne]['zz_lc_taille'] == null) $colonneAfficher = false;
	...
	private function colonnesTaille(){
		...
		if(!empty($this->liste['colonnes']['afficher'])){
			foreach($this->liste['colonnes']['afficher'] as $iColonne){
				$taille = $this->liste['colonnes']['proprietes'][$iColonne]['zz_lc_taille'];
				$colonnesTaille[] = ($taille ? $taille : 0);
			}//end foreach
		}//end if
