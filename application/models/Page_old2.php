...
class Agilis_Model_Page {
	...
	private function pageCadres(){
		$requete = "SELECT * FROM zz5_pagescadres INNER JOIN zz5_cadres ON ZZ_PC_Cadre = ZZ_CD_Code WHERE SUBSTRING_INDEX(zz_pc_code, '_', 3) = '".$this->page['code']."' ORDER BY ZZ_PC_Code";
	...
	private function pageControleur(){
		...
		if(! empty($this->page['cadres']) && $this->page['cadres'][0]['zz_pc_code'] == $this->page['code'].'_00'){
			$source = $this->page['cadres'][0]['zz_cd_source'];
			...
			$this->page['variables']['var_cadre_code'] = $this->page['cadres'][0]['zz_cd_code'];
		}//end if
